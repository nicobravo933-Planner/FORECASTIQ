...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="ForecastIQ — PySpark Benchmark Fase 11")
    parser.add_argument(
        "--local",
        action="store_true",
        default=False,
...
def build_features_spark(df_raw, spark):
    ...
    df = (
        ...
        .withColumn("es_fin_semana",
                    F.when(F.dayofweek("fecha").isin([1, 7]), 1).otherwise(0))
        .withColumn("precio_media_30d", F.mean("precio").over(w_30))
        .withColumn("precio_rel_30d",
                    F.col("precio") / (F.col("precio_media_30d") + 1e-6))
        .drop("precio_media_30d")
        .withColumn("cobertura_stock",
                    F.col("stock") / (F.col("ventas") + 1.0))
        .withColumn("cluster_abc_id",
                    F.when(F.col("cluster_abc") == "A", 0)
                     .when(F.col("cluster_abc") == "B", 1)
                     .otherwise(2))
        .withColumn("cluster_xyz_id",
                    F.when(F.col("cluster_xyz") == "X", 0)
                     .when(F.col("cluster_xyz") == "Y", 1)
                     .otherwise(2))
        .withColumn("categoria_id",
                    F.dense_rank().over(Window.orderBy("categoria")))
        .withColumn("canal_id",
                    F.dense_rank().over(Window.orderBy("canal")))
...
FEATURE_COLS = [
    ...
    "dia_semana", "mes", "trimestre", "semana_anio", "es_fin_semana",
    "precio_rel_30d", "cobertura_stock",
    "cluster_abc_id", "cluster_xyz_id", "categoria_id", "canal_id",
]
...
def train_and_eval_segment(df_train, df_test, abc: str, xyz: str) -> dict:
    ...
    dval   = lgb.Dataset(df_te[FEATURE_COLS].values, label=df_te["ventas"].values,
                         reference=dtrain)
...
def main() -> None:
    ...
    print(f"\n{'='*60}")
    print(f"ForecastIQ — PySpark Benchmark Fase 11")
    ...
    bench["polars_groupby"] = timed(
        lambda: df_pl.group_by("categoria").agg(pl.col("ventas").sum()), "Polars  groupby")
    ...
    df_pd_bench = df_pd[df_pd["sku_id"].isin(bench_skus)].sort_values(["sku_id", "fecha"])
    df_pl_bench = df_pl.filter(pl.col("sku_id").is_in(bench_skus)).sort(["sku_id", "fecha"])
    ...
    bench["pandas_rolling"] = timed(pandas_rolling, f"Pandas  rolling 7d ({len(df_pd_bench):,} rows)")
    bench["polars_rolling"] = timed(polars_rolling, f"Polars  rolling 7d ({len(df_pl_bench):,} rows)")
    ...
    if not args.skip_spark:
        ...
        bench["spark_rolling"] = timed(
            lambda: df_spark_bench.withColumn("rm7", F.mean("ventas").over(w_b)).count(),
    ...
    df_bench = pd.DataFrame({
        "Operación":     ["Read Parquet (27M rows)", "GroupBy + sum", "Rolling 7d (500k rows)"],
        "Pandas (s)":    [bench["pandas_read"],   bench["pandas_groupby"],   bench["pandas_rolling"]],
        "Polars (s)":    [bench["polars_read"],   bench["polars_groupby"],   bench["polars_rolling"]],
        "Spark (s)":     [bench["spark_read"],    bench["spark_groupby"],    bench["spark_rolling"]],
    }).set_index("Operación")
    ...
    if not args.skip_lgbm and spark is not None:
        ...
        fecha_max_str = (
            df_features.agg(F.max("fecha").alias("fm")).collect()[0]["fm"]
        )
        fecha_corte = (pd.Timestamp(str(fecha_max_str)) - pd.Timedelta(days=28)).strftime("%Y-%m-%d")
        ...
        for abc in ["A", "B", "C"]:
            for xyz in ["X", "Y", "Z"]:
                ...
                if metrics["wape"] is not None:
                    print(f"  {seg}  WAPE={metrics['wape']:.1f}%  "
