 ...
 * Columns: run name, model, freq, horizon, WAPE, MAE, date, Dagshub link.
 */
...
/** Returns chip color based on WAPE value. */
...
export function ExperimentTable({ runs, loading, error }: ExperimentTableProps) {
  if (loading) {
    return (
  ...
  if (error) {
    return (
      <Typography color="error" sx={{ py: "1.5rem" }}>
        {error}
      </Typography>
    );
  }
          ...
          {runs.map((run) => (
            ...
            >
              ...
              <TableCell>
                {run.wape !== null ? (
                  <Chip
                    label={`${run.wape.toFixed(1)}%`}
                    size="small"
                    color={wapeColor(run.wape)}
              ...
              <TableCell sx={{ pr: "0.5rem" }}>
                {run.dagshub_url ? (
                  <Tooltip title="Ver en Dagshub">
                    <IconButton
                      size="small"
                      href={run.dagshub_url}
                      target="_blank"
