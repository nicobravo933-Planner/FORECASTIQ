...
| 2026-05-25 | forecast-restore-fixes | **3 bugs relacionados a persistencia de resultados.** (1) `forecast/page.tsx`: `showResults` era `isDone && result != null` — al restaurar desde localStorage `stage=idle` nunca mostraba el resultado. Fix: `showResults = result != null`. Idle placeholder condicionado a `!forecast.result`. (2) `HomeDashboard`: `wape` y `fva` se mostraban como fracción (0.3%) en vez de porcentaje (32.4%) — el backend devuelve fracción, `MetricsCard` multiplica ×100 pero el Home no lo hacía. Fix: `wape * 100` y `fva * 100`. (3) `MiniChart` usaba `historical.slice(-6)` — solo 6 puntos, parecía hardcodeado. Fix: `slice(-24)` para mostrar hasta 24 períodos históricos reales. |
| 2026-05-25 | forecast-persist | **Forecast result persiste entre navegaciones.** `appStore`: nueva clave `fiq_last_forecast_result` + métodos `setLastResult/getLastResult/clearLastResult` (límite 500KB, silencia si excede). `useForecast`: inicializa estado con `getLastResult()` (lazy init) + llama `setLastResult(result)` en ambos paths de éxito (polling y eager). `HomeDashboard`: `useState` inicializa con `getLastResult()` + escucha `fiq:store-update` para actualizar en tiempo real; elimina fetch redundante al backend. |
| 2026-05-25 | home-chart-fix | Gráfico Home no se actualizaba al correr forecast en misma pestaña. Causa: `window.addEventListener("storage")` solo se dispara entre **tabs distintos** — en el mismo tab no hay evento. Fix: `appStore.setActiveJobId()` emite `CustomEvent("fiq:store-update")` + `HomeDashboard` escucha también ese evento además de `storage`. |
| 2026-05-25 | etl-storage-fix | **Bug 409 Duplicate** en `/winsorize`: `_save_supabase` usaba `upsert=false` → Supabase rechazaba la segunda escritura del mismo `_etl`. Fix: `upsert=true` en `storage.py`. **BeforeAfterChart**: agregadas `ReferenceLine` naranjas punteadas para `winsor_lower`/`winsor_upper` (solo en modo winsorize) — misma lógica visual que OutlierChart en EDA. Props `winsorLower`/`winsorUpper` pasadas desde `etl/page.tsx`. |
| 2026-05-25 | etl-slider-fix | Bug: WinsorizationPanel disparaba fetch en mount (modo local) antes de que el usuario moviera un slider. Fix: `isMountedRef` skipea el primer disparo del `useEffect` de debounce — ahora solo llama `onApply` en cambios reales del slider. |
