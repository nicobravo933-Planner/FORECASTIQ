...
| 2026-05-23 | 47      | Opción B separación cloud/local: pyproject.toml lightgbm+optuna+mlflow+evidently+nixtla+polars movidos a [dependency-groups] heavy-ml. Dockerfile liviano (sin gcc, sin heavy-ml). Dockerfile.worker idem. Dockerfile.worker-local nuevo (--group heavy-ml, queue=heavy). celery_app.py: imports lazy try/except para LightGBM, MLflow, Evidently — fallback HW en cloud. docker-compose.yml: celery_worker usa Dockerfile.worker + queue=default. |
