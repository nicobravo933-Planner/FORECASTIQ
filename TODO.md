...
| 2026-05-26 | signout-fixes       | `auth.ts` afterSignOut="/", Settings handleLogout → "/", botón Salir siempre visible + isAnonymous chip. |
...
| 2026-05-26 | landing-option-b | **Landing Opción B**: `page.tsx` reemplazado — `redirect("/landing.html")` siempre, sin chequeo de sesión. `LandingPage.tsx` ya no se usa como componente de ruta. `landing.html`: 3 botones CTA corregidos de `ForecastIQ Layout.html` → `/login`. F5 siempre vuelve a landing. |
| 2026-05-26 | landing-hydration-fix | **Hydration bug fix landing**: `<style>` JSX tag con comillas simples en `font-family` causaba mismatch SSR→client (`&#x27;` vs `'`). Fix: CSS extraído a `components/landing/landing.css` e importado como módulo. `<style>` tag eliminado de `LandingPage.tsx`. |
