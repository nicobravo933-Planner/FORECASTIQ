...
| 2026-05-23 | 57      | Refactor C completo (5 tareas): T1-DemoDatasetCard limpiado (sin tab Categoría, link a Analytics). T2-mini-chart histórico en /data (TextField SKU → AreaChart Recharts, tab "Serie: SKU-XXXXX", botón Ir a Forecast). T3-Analytics drill-down real (handleDrillDown → demo/load → appStore → /forecast, 1 paso). T4-ForecastConfigPanel model lock por tier (LightGBM con LockIcon+tooltip cuando caps.tier=cloud). T5-appStore detectedModel cache (setDetectedModel/getDetectedModel/clearDetectedModel, pre-select en /forecast al volver). |
