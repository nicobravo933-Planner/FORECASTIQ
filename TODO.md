...
| 2026-05-26 | skills-debug-setup  | `SKILL.md` personal en raíz del proyecto (skill de verificación para Claude Projects). `scripts/debug/` creado con `.gitkeep`. `.gitignore` actualizado: excluye `scripts/debug/*.py` y `/SKILL.md`. |
| 2026-05-26 | deploy-fix          | Bug deploy EC2: (1) `sleep 8` insuficiente → retry loop 15s+3×5s en `deploy.yml`. (2) `deploy.replicas: 0` ignorado por Compose v2 (es sintaxis Swarm) → reemplazado por `profiles: [local]` en `docker-compose.prod.yml`. Workers Celery ya no levantan en EC2. (3) **causa raíz**: `uv run` disparaba re-sync de deps al arrancar el container (mypy, ruff 14-10 MB) causando delay 30-60s. Fix: `ENV PATH="/app/.venv/bin:$PATH"` + `CMD ["uvicorn", ...]` directo en `Dockerfile` y `Dockerfile.worker`. |
